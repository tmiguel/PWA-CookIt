...
const validate = (code, name) => {
    ...
    return { code: cleanCode, name: cleanName, updatedAt: serverTimestamp() }; // Sem createAt no update
};
...
export async function addTag(code, name) {
    ...
    data.createdAt = serverTimestamp(); // Só adicionamos no criar
...
// NOVA FUNÇÃO: EDITAR
export async function updateTag(id, code, name) {
    const data = validate(code, name);
    const ref = doc(db, COLLECTION, id);
    await updateDoc(ref, data);
}

export async function deleteTag(id) { await deleteDoc(doc(db, COLLECTION, id)); }
